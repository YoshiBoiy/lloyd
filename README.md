...
| OpenAI | `OPENAI_API_KEY`, `OPENAI_MODEL` | Scripted bounded planner | Responses API strict function proposal, no storage, only permitted context. Case query ID is replaced by a placeholder before model planning. |
...
npm run lint:edge
npm run test:edge
npm run test:smoke        # starts a disposable local Node API and releases the Python fixture over HTTP
npm audit
```
